    ...
    function enterFullscreen(imageSrc) {
        ...
        document.addEventListener('fullscreenchange', function () {
            if (!document.fullscreenElement) {
                ...
                document.documentElement.removeChild(imageOverlay);
            }
        });
        imageOverlay.addEventListener('click', exitFullscreen);
        ...
        let scale = 1;
        imageOverlay.addEventListener('wheel', function (event) {
        event.preventDefault();
        scale += event.deltaY * -0.01;
        scale = Math.min(Math.max(0.1, scale), 3);
        imageOverlay.querySelector('img').style.transform = `scale(${scale})`;
        });
        }
...
function del_com(id){
    ...
    $.ajax({
        type: "POST",
        url: "/Vision/delcommentvision/",
        data: {
            'visioncomID': visioncomID,
            'csrfmiddlewaretoken': csrfToken
        },
        dataType: 'json',
        contentType: "application/x-www-form-urlencoded; charset=utf-8",
        success: function(result) {
            comid = 'com' + visioncomID
